        ...
        onMessageReceived: (message) {
          try {
            ...
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                builder: (context) => SuccessScreen(response: response),
              ),
            );
          } catch (e) {
            debugPrint('Invalid response format: ${message.message}');
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Invalid response from device')),
            );
          }
        },
      )
